...
#chemFile <- "TemplatePhysChemFile.txt"           # used for comparing results from running R scripts
...
modelType <-  "solve_pbtk"                    # "solve_pbtk", "solve_fetal_pbtk", or "solve_gas_pbtk"
```
...
```{r}
route <- "oral"      # options are "inhalation", "iv","oral"; "solve_gas_pbtk" uses the inhalation (gas) route,
...
chemical$logPwa<--1*(chemical$HL+1.61) #this gets the water-air partition coefficient, as the unit for HL from OPERA is (LogHL,atm-m^3/mole),logaw=logHL-logRT=logHL-log(0.0244)for T=298.15K  #2025
...
chem.physical_and_invitro.data["50-00-0", "Compound"] <- 'Formaldehyde'         #2025
...
if(modelType == "solve_pbtk"){
  for(this.cas in chemical[,"CASRN"]) {
    set.seed(1234)                                                                #2025
    Kkidney2pu <- chemical$Kkidney2pu[chemical$CASRN == this.cas]                 #2025June, place it at the top level rather than repeating it under each model type
    Kliver2pu  <- chemical$Kliver2pu[chemical$CASRN == this.cas]                  #2025June, place it at the top level rather than repeating it under each model type
    Krest2pu   <- chemical$Krest2pu[chemical$CASRN == this.cas]                   #2025June, place it at the top level rather than repeating it under each model type
    Kgut2pu    <- chemical$Kgut2pu[chemical$CASRN == this.cas]                    #2025June, place it at the top level rather than repeating it under each model type
    print(this.cas)
    half_life <- calc_half_life(chem.cas = this.cas, species = species_1)         #Half life calculated from the httk function; independent of PBPK model type; it is calculated from the httk 3-compartment steady state model; 
    ...
    output0 <- solve_pbtk(chem.cas = this.cas, parameters = param, doses.per.day = dpd, days = ndays, tsteps = 4, dose=0, daily.dose = expDose*dpd, iv.dose = iv.dose, output.units = ConcentrationUnit, species = species_1, default.to.human = TRUE, plots = F, suppress.messages = TRUE)
    output0 <- as.data.frame(output0)
    maxAUC.h <- max(output0$AUC)*24
    #steady state concentration as a result of infusion dosing for the solve_pbtk model
    CSS0 <- calc_analytic_css(chem.cas = this.cas, parameters = param, daily.dose = expDose*dpd, doses.per.day = dpd, route=route, species = species_1, model = 'pbtk', output.units = ConcentrationUnit)
...
if(modelType == "solve_gas_pbtk"){
  ...
  for(this.cas in chemical[,"CASRN"]) {
    ...
    param <- parameterize_gas_pbtk(chem.cas = this.cas, species = species_1, default.to.human = TRUE) #2025
    param$Kkidney2pu <- ifelse(!is.na(Kkidney2pu), Kkidney2pu, param$Kkidney2pu)            #2025June
    param$Kkidney2pu_Source <- ifelse(!is.na(Kkidney2pu), "CUSTOM", "httkCalculated")       #2025June 
    param$Kliver2pu <- ifelse(!is.na(Kliver2pu), Kliver2pu , param$Kliver2pu )              #2025June
    param$Kliver2pu_Source <- ifelse(!is.na(Kliver2pu), "CUSTOM", "httkCalculated")         #2025June
    ...
    output0 <- solve_gas_pbtk(chem.cas = this.cas, parameters = param, doses.per.day = NULL, days = ndays, tsteps = 4, dose=NULL, daily.dose = NULL, exp.conc = expConc, period=interv, input.units = gasInputUnit, exp.duration = expLength, output.units = ConcentrationUnit, species = species_1, default.to.human = TRUE, plots = F, suppress.messages = TRUE)
    output0 <- as.data.frame(output0)
    maxAUC.h <- max(output0$AUC)*24
    concs <- output0 %>% select(c("time", "Cgut", "Cliver", "Cven", "Clung", "Cart", "Crest", "Ckidney", "Cplasma"))       
   }
...
if(modelType == "solve_fetal_pbtk"){
  ...
  for(this.cas in chemical[,"CASRN"]) {
    ...
    param <- parameterize_fetal_pbtk(chem.cas = this.cas, species = species_1, default.to.human = TRUE)
    param$pre_pregnant_BW <- bWeight  #Default 61.1 kg, not tied to physiology.data #2025Sept
    print(param$pre_pregnant_BW)      #2025Sept
    #print(param$BW)  # to make sure that the body weight had been changed.
    param$Kkidney2pu <- ifelse(!is.na(Kkidney2pu), Kkidney2pu, param$Kkidney2pu)            #2025June
    param$Kkidney2pu_Source <- ifelse(!is.na(Kkidney2pu), "CUSTOM", "httkCalculated")       #2025June 
    param$Kliver2pu <- ifelse(!is.na(Kliver2pu), Kliver2pu , param$Kliver2pu )              #2025June
    ...
    output_f <- solve_fetal_pbtk(chem.cas = this.cas, parameters = param, times=seq(gestationDays,gestationDays+ndays,0.025), doses.per.day = dpd, dose= NULL, daily.dose = expDose*dpd, iv.dose = iv.dose, output.units = ConcentrationUnit, species = "human", default.to.human = TRUE, plots = FALSE, suppress.messages = TRUE) 
...
param_table<-param_table %>% select(any_of(c("CASRN", "ChemicalName", "DTXSID", "Flag", "Species", "Model",
                               "Route", "Dose, mg/kg", "Exposure Concentration", "Exposure Concentration Unit", "Exposure Length, hr", "Exposure Period, hr", "Dose Interval, hrs",
                               "Length of Dosing, Days", "Clint","Clint Source", "Clint_InputSource", "fu", "fu Source", "fu_InputSource", "MW", "MW_InputSourse",
                               "LogP", "LogP_InputSource", "HL", "HL_InputSource", "pka_Accept", "pka_Accept_InputSource", "pka_Donor", "pka_Donor_InputSource")))
#colnames(param_table)<-gsub("pka_Accept", "pka Acceptor", colnames(param_table))              #2025June Commented, as it is unnecessary
#colnames(param_table)<-gsub("pka_Donor", "pka Donor", colnames(param_table))                  #2025June Commented, as it is unnecessary
param_table <- left_join(param_table, p.out_all, by = "CASRN")                                 #2025
```
