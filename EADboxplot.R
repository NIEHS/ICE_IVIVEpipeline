EADboxplot <- function(EAD.out, invivo = NULL, label = "EAD",chemDisplay="CASRN", EADUnit="mg/kg/dose",modelType = "1C", species="human", route = "iv", date = "2019" ,doses.per.day = 1, wth=0.2, axis.text=10, title.text=14, shape.size=5, dpi=300)
{
  ...
  selEAD <- cbind(EAD.out$CASRN, EAD.out$CASRN, seldata)
  ...
  if(!is.null(invivo))
  {
    ...
    labs(x='Chemical')
    plot1
    ggsave(plot1, file=paste(label, "_invivo_", modelType, "_", species, "_", route, "_dpd", doses.per.day, "_", date.string, ".jpeg"), width=7,height=5, dpi=dpi,units = "in", device='jpeg')
  }
  plot1
