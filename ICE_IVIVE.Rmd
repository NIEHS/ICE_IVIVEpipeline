...
modelType <-  "solve_pbtk" # "1C", solve_3comp", "solve_pbtk", "solve_gas_pbtk"
```
...
```{r}
gasDosing <-"expConc" # two dosing methods for Solve_gas_pbtk model, 
                      # "expConc" and "expDose"
...
source("CalcEAD.R") # required for "1C", "solve_3comp", 
                    # "solve_pbtk", and "solve_gas_pbpk" models
source("EADboxplot.R")
```
...
chemical <- as.data.frame(read_delim(chemFile, delim = "\t")) #there are single quotes 
                    #in chemical names that need to be addressed, the tidyR handels well
chemical[1:2,]
```
...
chemical$Clint<-10^chemical$Clint #this moves from log10 ul/ml/10^6 
                                  #cells to just ul/ml/10^6 cells
...
This will run a simple, 1 compartment model. In this model the concenctration at steady state is determined vs the maximum concentration from the other models.
...
The [solve_gas_pbtk](https://rdrr.io/cran/httk/man/solve_gas_pbtk.html) is a PBPK model similar to the "solve_pbtk" model but is uses an inhalation route of exposure assuming the chemical is volatile (gas). As a result, it has some additional checks to see if the assumption of gas exposure is reasonable but will still proceed with a flag warning. It also includes 2 different dosing approaches, a concentration over time (expConc) or a single dose (expDose).
  ...
  for(this.cas in chemical[,"CASRN"]) {
    ...
    if(modelType == "solve_gas_pbtk"& gasDosing == "expConc"){
      ConcentrationUnit <- "uM"  # output unit only has one option 
                                 # of "uM" for Solve_gas_pbtk model
