...
modelType <-  "solve_fetal_pbtk" #"1C", solve_3comp", "solve_pbtk", "solve_gas_pbtk", "solve_fetal_pbtk"
```
...
if(modelType != "solve_gas_pbtk"){
  EADUnit <- "mg/kg/dose"
                                ...
                                #Argument for EADboxplot.R, "mg/kg/dose" for "1C", "solve_3comp", "solve_pbtk"; 
                                #uM or ppmv for "solve_gas_pbpk" model
...
if(modelType == "solve_fetal_pbtk"){
  ...
  Cmax <- merge(chemical, cmaxall, by.x="CASRN", by.y="this.cas")
  names(Cmax) <- gsub("concMax", "Cmax", names(Cmax) )
  #Calculating the EADs corresponding to maternal 
  EAD.out_max <- CalcEAD(Css = Cmax[,c("CASRN", "Cmax")], inVitro = invitro)
  EAD.out_fmax <- CalcEAD(Css = Cmax[,c("CASRN", "fCmax")], inVitro = invitro)
  colnames(EAD.out_fmax)<-gsub("EAD","fetal_EAD", colnames(EAD.out_fmax)) #rename EAD to fetal EAD which corresponds to fetal plasma Cmax; this is the same as "EAD.Fmax.50th" shown in the ICE4.0 IVIVE tool
  EAD.out_both<-left_join(EAD.out_max, EAD.out_fmax)
  ...
  CSS2$"Length of Dosing, Days" <- ndays
  #reformatting columns to match the other models:
  CSS2<-CSS2 %>% select(any_of(c("CASRN", "ChemicalName", "Cmax", "fCmax", "Cmax, Units","Flag", "Species","Model",
  "Route","Dose Interval, hrs","Length of Dosing, Days","Clint","Clint Source", "fu","fu Source" ,"MW")))
  ...
  #plotting
  EADplot <- EADboxplot(EAD.out = outputData, invivo = invivo, label="EAD", EADUnit = EADUnit, species = species, route = route, modelType = modelType, doses.per.day = dpd, date = date, chemDisplay = chemDisplay, axis.text=4) 
  fEADplot <- EADboxplot(EAD.out = outputData, invivo = invivo, label="fetal_EAD", EADUnit = EADUnit, species = species, route = route, modelType = modelType, doses.per.day = dpd, date = date, chemDisplay = chemDisplay, axis.text=4) 
  #Arguments for EADplot functions include "EAD.out, invivo, label, chemDisplay, EADUnit, modelType, species, route, date, doses.per.day, wth, axis.text, title.text, shape.size, dpi"
  print(EADplot)
  print(fEADplot)
}
